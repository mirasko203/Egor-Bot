import telebot              # pyTelegramBotAPI — для работы с Telegram-ботом
...
import os                   # Чтобы получать порт, который выдаёт Render
import sqlite3              # Для хранения лайков/дизлайков в локальной базе

# ----------------- TELEGRAM BOT -----------------
TOKEN = '7772407762:AAHwJ0y5b-gcHZG6xd832_c2NyF98OY5m08'  # Токен бота от BotFather
bot = telebot.TeleBot(TOKEN)  # Создаём объект бота

# ----------------- SQLITE -----------------
conn = sqlite3.connect('bot.db', check_same_thread=False)  # Подключаемся к базе bot.db
cursor = conn.cursor()

# Создаём таблицу для стихов с лайками/дизлайками, если её ещё нет
cursor.execute('''
...
# Добавляем стихи в базу, если их ещё нет
poems = [
    (1, 'День учителя'),
    (2, 'Крушение "Ан-24"'),
    (3, 'Донбасс'),
...
def start(message):
    ...
    keyboard.add(btn1, btn2)  # Добавляем кнопки на клавиатуру
...
def callback(call):
    ...
    # ----------------- ВЫБОР СТИХОВ -----------------
        ...
        bot.answer_callback_query(call.id)
        markup = types.InlineKeyboardMarkup()  # Клавиатура для выбора стихов

        # Получаем текущее количество лайков/дизлайков для каждого стиха
        cursor.execute("SELECT likes, dislikes FROM poems WHERE id = 1")
        likes1, dislikes1 = cursor.fetchone()
        cursor.execute("SELECT likes, dislikes FROM poems WHERE id = 2")
        likes2, dislikes2 = cursor.fetchone()
        cursor.execute("SELECT likes, dislikes FROM poems WHERE id = 3")
        likes3, dislikes3 = cursor.fetchone()
        cursor.execute("SELECT likes, dislikes FROM poems WHERE id = 4")
        likes4, dislikes4 = cursor.fetchone()

        # Создаём кнопки с названием стиха + лайки/дизлайки
        p1 = types.InlineKeyboardButton(text=f'День учителя 👍{likes1} 👎{dislikes1}', callback_data="p1")
        p2 = types.InlineKeyboardButton(text=f'Крушение "Ан-24" 👍{likes2} 👎{dislikes2}', callback_data='p2')
        p3 = types.InlineKeyboardButton(text=f'Донбасс 👍{likes3} 👎{dislikes3}', callback_data='p3')
        p4 = types.InlineKeyboardButton(text=f'Таня Савичева 👍{likes4} 👎{dislikes4}', callback_data='p4')
    ...
    # ----------------- СТИХИ + КНОПКИ ЛАЙК/ДИЗЛАЙК -----------------
    elif call.data.startswith('p'):  # Любой стих
        bot.answer_callback_query(call.id)
        ...
        # Кнопки лайк/дизлайк под стихом
        markup = types.InlineKeyboardMarkup()
        like_btn = types.InlineKeyboardButton(text="👍", callback_data=f"like_{poem_id}")
        dislike_btn = types.InlineKeyboardButton(text="👎", callback_data=f"dislike_{poem_id}")
...
# ----------------- FLASK — ДЛЯ RENDER -----------------
app = Flask(__name__)  # Создаём объект Flask

@app.route("/")  # Главная страница веб-сервера
def home():
    return "Bot is alive!"  # Render видит открытый порт

def run_flask():  # Функция запуска Flask
    port = int(os.environ.get("PORT", 10000))  # Получаем порт Render
    app.run(
        host="0.0.0.0",     # Слушаем на всех интерфейсах
        port=port,          # Порт Render
        debug=False,        # Без режима отладки
        use_reloader=False  # Без автоматической перезагрузки
    )

# ----------------- ЗАПУСК -----------------
# Сначала Flask, чтобы Render увидел порт
